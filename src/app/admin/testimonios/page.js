...
export default function AdminTestimoniosPage() {
  ...
  const [formData, setFormData] = useState({
    type: 'testimonial', // 'testimonial', 'transformation', 'quote'
    name: '',
    memberSince: '',
    testimonial: '',
    image: '',
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // Validar límites solo al crear nuevos (no al editar)
      if (!editingTestimonio) {
        const countByType = {
          testimonial: testimonios.filter(t => t.type === 'testimonial' || !t.type).length,
          transformation: testimonios.filter(t => t.type === 'transformation').length,
          quote: testimonios.filter(t => t.type === 'quote').length,
        };
        ...
        if (countByType[formData.type] >= limits[formData.type]) {
          const typeNames = {
            testimonial: 'Testimonios Auténticos',
            transformation: 'Transformaciones Reales',
            quote: 'Frases Motivadoras',
          };
          alert(`Ya tienes el máximo de ${limits[formData.type]} ${typeNames[formData.type]}. Edita o elimina uno existente.`);
          return;
        }
      }
  ...
  const handleEdit = (testimonio) => {
    ...
    setFormData({
      type: testimonio.type || 'testimonial',
      ...
      beforeImage: testimonio.beforeImage || '',
      afterImage: testimonio.afterImage || '',
  ...
  return (
    <div className="min-h-screen bg-gray-950">
      ...
      <AdminHeader 
        ...
        subtitle="Gestiona testimonios y transformaciones de clientes"
      />
            ...
            <div key={testimonio.id} className="bg-gray-900 rounded-xl p-6 border border-gray-800 hover:border-avc-red transition-all duration-300 border-l-4 border-l-avc-red">
              ...
              <div className="mb-4">
                <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold uppercase ${
                  testimonio.type === 'transformation' 
                    ? 'bg-purple-600 text-white' 
                    : testimonio.type === 'quote'
                    ? 'bg-yellow-600 text-white'
                    : 'bg-blue-600 text-white'
                }`}>
                  {testimonio.type === 'transformation' ? '🔥 Transformación' 
                    : testimonio.type === 'quote' ? '💬 Frase' 
                    : '⭐ Testimonio'}
                </span>
              </div>
                ...
                <>
                  <div className="grid grid-cols-2 gap-2 mb-4">
                    <div className="relative h-40">
                      <Image
                        src={testimonio.beforeImage || 'https://placehold.co/300x300/444444/white?text=ANTES'}
                        alt="Antes"
                        fill
                    ...
                    <div className="relative h-40">
                      <Image
                        src={testimonio.afterImage || 'https://placehold.co/300x300/777777/white?text=DESPUÉS'}
                        alt="Después"
                        fill
                        className="rounded object-cover"
                      />
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div>
                ...
                >
                  ...
                  <option value="quote">💬 Frase Motivadora (máx 3)</option>
                </select>
              </div>
