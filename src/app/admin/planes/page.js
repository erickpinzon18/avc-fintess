      ...
      <main className="container mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {planes.map((plan) => (
            <div key={plan.id} className="bg-gray-900 rounded-xl p-6 border border-gray-800 hover:border-avc-red transition-all duration-300 relative">
              {plan.popular && (
                <div className="absolute top-4 right-4 bg-avc-red text-white px-3 py-1 rounded-full text-xs font-bold">
                  POPULAR
                </div>
              )}
        ...
        {planes.length === 0 && (
          <div className="text-center py-20">
            <p className="text-gray-400 text-lg mb-4">No hay planes registrados</p>
            <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-gray-900 rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              <div>
                <label className="block text-sm font-semibold text-gray-300 mb-2">Nombre del Plan</label>
                <input
                  type="text"
                  value={formData.name}
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-sm font-semibold text-gray-300">Características</label>
                  <button
                    type="button"
                    onClick={addFeature}
